...
public class OrderInfoController {
	...
	//根据页码检索商品信息
		...
		@ResponseBody
		public JsonResult  addOrderItemLink(HttpSession session, Long orderId, Long[] id1) {
			Long ownerId = ((UserInfo)(session.getAttribute("user"))).getOwnerId();
			Long operatorId = ((UserInfo)(session.getAttribute("user"))).getId();
			Date tamp = new Date(System.currentTimeMillis());
			Byte authorized = new Byte("1");
			service.addOrderItemLink(ownerId, tamp, operatorId, orderId, id1, authorized);
			return new JsonResult();
		}
		 //根据订单状态查询
		...
		@RequestMapping("/modifyOrderInfo1.do")
		@ResponseBody
		public JsonResult  modifyOrderInfo1(Long id,String userName, String userTelephone,
				String userAddress,HttpSession session,Date gmtModified, String note) {
			Long operatorId = ((OrderInfo)(session.getAttribute("user"))).getId();
			Date tamp = new Date(System.currentTimeMillis());
			int n=service.modifyOrderInfo1(id, ownerId, userName, userTelephone, userAddress,operatorId, tamp, note);
			return new JsonResult(n);
		}
