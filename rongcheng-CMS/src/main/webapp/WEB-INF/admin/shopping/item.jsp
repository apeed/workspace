   		...
   		$.ajax({
   			...
   			success : function(result) {
   				...
   				if(page>maxPage){page = maxPage;}
   					...
   					nowPage = page;
   					reloadTable(result);
   					pagination(maxPage,page,"pageTd","loadItem");
   			},
   			error : function(data) {
   	...
   	function reloadTable(data){
		...
		var itemInfoList = data.itemInfoList;
 		for(var i = 0;i<iiLength;i++){
 	        ...
 	        str+='<td><button class="button border-main recommend_btn" onclick="recommendBtn(0,'+itemInfo.id+')" type="button">归类到</button></td>';
 	        str+='<td>';
   	  ...
   	  if(confirm("您确定要删除吗?")){
   		...
   		$.ajax({
	   		...
	   		flag=true;
   				...
   				if (maxPage != 0){
   					reloadTable(result);
   					pagination(maxPage,nowPage,"pageTd","loadItem");
   				}else{
   					$(".jxb_removeTr").remove();
   				}
   			},
