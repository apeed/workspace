	...
	function recommendDel(){
   		...
   		$.ajax({
   			...
   			success : function(result) {
   				...
   				if(page>maxPage){page = maxPage;}
   					...
   					nowPage = page;
   					reloadTable(result);
   					pagination(maxPage,page,"pageTd","loadContent");
   			},
   			error : function(data) {
   	...
   	function reloadTable(data){
   		...
   		for(var i = 0;i<data.content.length;i++){
   			...
   			str+='<td><input type="checkbox" name="id" value="'+data.content[i].id+'"/>'+data.content[i].id+'</td>';
   			str+='<td>'+data.content[i].contentTitle+'</td>';
   	        str+='<td>页面内容</td>';
   	        str+='<td>'+(!data.content[i].note?"":data.content[i].note)+'</td>';
   	        str+='<td>'+new Date(data.content[i].gmtModified).format("yyyy-MM-dd hh:mm:ss")+'</td>';
   	        str+='<td><button class="button border-main recommend_btn" type="button" onclick="recommendBtn(0,'+data.content[i].id+')">归类到</button></td>';
   	        str+='<td>';
   	          str+='<div class="button-group"> ';
   	        	str+='<a class="button border-main" href="/admin/content/modifycontent.do?id='+data.content[i].id+'"><span class="icon-edit"></span> 修改</a>';
   	        	str+='<a class="button border-red" href="javascript:void(0)" onclick="del('+data.content[i].id+')"><span class="icon-trash-o"></span> 删除</a>';
   	       	  str+='</div>';
   	        str+='</td>';
   	        str+='</tr>';
   	  ...
   	  if(confirm("您确定要删除吗?")){
   		...
   		$.ajax({
   			...
   			success : function(result) {
   				...
   				if (maxPage != 0){
   					reloadTable(result);
   					pagination(maxPage,nowPage,"pageTd","loadContent");
   				}else{
   					$(".jxb_removeTr").remove();
   				}
   			},
    ...
    <div class="panel admin-panel">
      ...
      <div class="padding border-bottom">
        <ul class="search" style="padding-left:10px;">
          <li> <a class="button border-main icon-plus-square-o" href="/admin/content/addcontent.do"> 添加内容</a> </li>
          <li>
          ...
          <li>
            ...
            <a href="javascript:void(0)" class="button border-main icon-search" onclick="loadContent(1)" > 搜索</a></li>
        </ul>
      </div>
      		<form id="subForm">
