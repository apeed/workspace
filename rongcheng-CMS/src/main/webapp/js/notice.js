...
function changesearch() {
	...
	$.ajax({
		url : "findListByStatementNotice.do",
		type : "post",
		data : {
			"keyword" : keywords
		},
...
function reload(list) {
	...
	for(var i=0;i<list.length;i++){
		...
		var reserved1 = StatementNotice.reserved1;
		...
		str+='<td>'+operatorId+'</td>';
		str+='<td><div class="w400_gsgg">'+statement+'</div></td>';
		str+='<td>'+note+'</td>';
		str+='<td>'+time+'</td>';
		str+='<td><div class="button-group">';
		if(reserved1==1){
			str+='<a href="javascript:;" class="button border-main isStartUsing" id="useIt" style="background-color:rgb(0, 170, 238);color:#FFF">启用</a>';
		}else{
...
//	addCookie("reserved1", reserved1, 1);
//	console.log(getCookie("reserved1"));
	/////页码////////////
	var trs="";
	trs += '<tr id = "page_tr">';
	trs += '<td colspan="10"><div class="pagelist"><a  onclick="loadNotice(1)" href="javascript:;">首页</a><a class="bookpage" onclick="loadNotice('+(now_page-1)+')" href="javascript:;">上一页</a>';
	if(max_page>5){
		if(now_page<=3){
			for(var i=1;i<4;i++){
				if(i==now_page){
