...
function changesearch() {
	...
	$.ajax({
		url : "findListByCargoTracking.do",
		type : "post",
		data : {
			"keyword" : keywords
		},
...
function reload(list) {
	...
	var trs="";
	trs += '<tr id = "page_tr">';
	trs += '<td colspan="10"><div class="pagelist"><a onclick="loadLogistics(1)" href="javascript:;">首页</a><a class="bookpage" onclick="loadLogistics('+(now_page-1)+')" href="javascript:;">上一页</a>';
	if(max_page>5){
		if(now_page<=3){
			for(var i=1;i<4;i++){
				if(i==now_page){
