...
function loadModify(){
		var CargoTrackingId = getCookie("cid");
		$.ajax({
			url : "load_modify_cargoTracking.do",
			type : "post",
			...
			success:function(result){
				if (result.state == 0) {
					...
					var logisticsPublic_transferCarrierId=$("#logisticsPublic_transferCarrierId").val(transferCarrierId);
