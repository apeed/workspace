 ...
 * @author 赵滨
 */
function recommendBtn(GLType,id) {
	
	console.log(GLType,id);
...
function createBlockChannelBatch(listBlockChannel, nowPage, rows) {
	...
	for (var i = 0; i < blockChannelList.length; i++) {
		...
		if (blockChannelList[i].parentId == 0) {
			...
			put += '<li><input type="checkbox" name="blockChannelId" value="'+blockChannelList[i].id+'">';
		...
		for (var i = 0; i < blockChannelList.length; i++) {
			...
			for (var j = 0; j < webPageBlockChannelList.length; j++) {
				...
				if (blockChannelParentId == webPageBlockChannelId) {
					...
					put += '<ul>';
					put += '<li><input type="checkbox" name="blockChannelId" value="'+blockChannelList[i].id+'"><span class="column_span1">';
					put += blockChannelList[i].userSequence;	//序号
					put += '</span></li>';
...
function createBlockChannelAll(listBlockChannel, nowPage, rows) {
	...
	put += '<li style="width: 30%" class="">选择</li>';
	put += '<li style="width: 70%" class="">栏目名称</li>';
		...
		for (var i = 0; i < blockChannelList.length; i++) {
			...
			for (var j = 0; j < webPageBlockChannelList.length; j++) {
				...
				if (blockChannelParentId == webPageBlockChannelId) {
					...
					put += '<ul>';
					put += '<li style="width: 30%"><span class="column_span1">';
					put += blockChannelList[i].userSequence;	//序号
					put += '</span></li>';
					put += '<li  style="padding-left: ';
					put += 25 * ratio;
