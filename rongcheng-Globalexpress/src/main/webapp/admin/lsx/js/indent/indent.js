	...
	/*$("#la").mouseover(function(){
	 ...
	 $("#la").mouseout(function(){
         if (orderNum.length > 5) { 
        	 orderNum = orderNum.substring(0,5)+"..."}
         $("#la").html(orderNum);
     });*/
});
...
function fukuancit() {
	...
	$.ajax({
		url:"/admin/findListByOrderType.do",
		type:"post",
		dataType:"json",
...
function changesearch(){
	...
	$.ajax({
		url : "/admin/findListByOrderInfo.do",
		type : "post",
		data : {
			"keyword" : keywords
		},
...
//		console.log(orderType);
	...
	//发货	
		...
		if(orderType==1){
			...
			str+='<a class="button border-red indent_del_button"  href="javascript:;" onclick="return del('+id+',this)"><span class="icon-trash-o"></span> 删除</a>';
