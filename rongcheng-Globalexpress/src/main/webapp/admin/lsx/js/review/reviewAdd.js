...
function loadSome() {
	var strs="";
	strs+='<input type="text" class="input" value="" name="itemSku_input" placeholder="请输入商品编号或名称搜索"/>';
	strs+='<ul class="search_xiala_box" id="review_itemSku">';
	strs+='</ul>';
	strs+='<span class="tips" id="itemSku_div_ts" style="color:red"></span>';
	$("#itemSku_div").append(strs);
	var st="";
	st+='<input type="text" class="input" value="" name="accountNum_input"  placeholder="请输入客户账号或姓名搜索"/>';
	st+='<ul class="search_xiala_box" id="review_accountNum">';
	st+='</ul>';
	st+='<span class="tips" id="accountNum_div_ts" style="color:red"></span>';
...
//	}
	if(review_userAccount==""){
		ok=false;
		$("#accountNum_div_ts").html("客户账户为空");	
	}else{
		$("#accountNum_div_ts").html("");	
	}
	/*if(review_userTel==""){
		ok=false;
		$("#review_userTel_ts").html("客户电话为空");
	}else if(!/^(0|1)\d{10,11}$/.test(review_userTel)){
		ok=false;
