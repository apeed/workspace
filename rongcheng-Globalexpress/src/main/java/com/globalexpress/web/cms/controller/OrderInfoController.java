...
//	private Long ownerId;
	...
	//根据页码检索商品信息
		...
		public JsonResult addOrderInfo(HttpSession session,Long orderNum, Long consignorId,
			...
			int n=service.addOrderInfo(ownerId, tamp, operatorId, orderNum, consignorId, consigneeId, consigneeTel, consigneeAddress, orderLabel, carrierId, trackingNum, packageWeight, totalShippingFee,orderKeyword,supportValue,note,new Byte("1"));
		...
		//查找所有的订单编号
		@RequestMapping("/findOrderNums.do")
		@ResponseBody
		public JsonResult findOrderNums(HttpSession session) {
			Long ownerId = ((UserInfo)(session.getAttribute("user"))).getOwnerId();
